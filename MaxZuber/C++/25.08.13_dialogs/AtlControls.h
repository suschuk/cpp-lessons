...
template <class Base> class CGripT;
...
////////////////////////////////////////////////////////////////////////////////////////////////
//
// ESVN List View control
//

template < class T>
class CEsvnListViewCtrlT : public CWindowImpl< CEsvnListViewCtrlT< T >, CListViewCtrl>
{
public:
    BEGIN_MSG_MAP(CEsvnListViewCtrlT< T >)
        MESSAGE_HANDLER(WM_SIZE, OnSize)
    END_MSG_MAP()

    // Constructors
    CEsvnListViewCtrlT (HWND hWnd = NULL)
    {
        m_hWnd = hWnd;
    }

    CEsvnListViewCtrlT< CListViewCtrl >& operator=(HWND hWnd)
    {
        m_hWnd = hWnd;
        return *this;
    }
    ...
    LRESULT OnSize(UINT /* uMsg */, WPARAM /* wParam */, LPARAM /* lParam */, BOOL& /* bHandled */)
    {
        ...
        for(int i = 1; i < ((CHeaderCtrl)GetHeader()).GetItemCount(); ++i)
            reserved_width += GetColumnWidth(i);
        SetColumnWidth(0, rc.right - reserved_width);
...
typedef CEsvnListViewCtrlT<CWindow> CEsvnListViewCtrl;
...
/////////////////////////////////////////////////////////////////////////////
// CGrip - Grip control wrapper.
template <class Base>
class CGripT : public Base
{
public:
    CGripT(HWND hWnd = NULL) : Base(hWnd) { }
    CGripT< Base >& operator=(HWND hWnd)
    {
        m_hWnd = hWnd;
        return *this;
    }
    HWND Create(HWND hWndParent, RECT& rcPos, LPCTSTR szWindowName = NULL,
            DWORD dwStyle = WS_CHILD | WS_VISIBLE | SBS_SIZEGRIP, DWORD dwExStyle = 0,
            UINT nID = 0, LPVOID lpCreateParam = NULL)
    {
        return CWindow::Create(GetWndClassName(), hWndParent, rcPos, szWindowName, dwStyle, dwExStyle, nID, lpCreateParam);
    }
